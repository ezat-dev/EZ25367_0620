	   ...
	   <div id="modalContainer" class="modal">
	    <div class="modal-content">
	        ...
	        <form id="corrForm"autocomplete="off">
	            <label>설비명</label>
            <select id="popup_equip" name="equipment_name" >
                 <option value="열처리 연속로">열처리 연속로</option>
		...
		<!-- 		<input type="hidden" class="facility_equipment_name" name="equipment_name" value=""> -->
				<input type="hidden" class="facility_standard_code" name="standard_code" value="">
...
$(document).ready(function() {
    ...
    $(".select-button").click(function() {
        ...
        dataListTable.setData("/chunil/standard/nonTime/list", {
            equipment_name: equipment_name,
            startDate: startDate,
            endDate: endDate
        });

        dataViewTable.setData("/chunil/standard/nonTime/view", {
            equipment_name: equipment_name,
            startDate: startDate,
            endDate: endDate
        });
...
function openModal(mode, rowData) {
    ...
    if (mode === "edit") {
        ...
        $form.find("select[name='equipment_name']").val(rowData.standard_code);
        ...
        if ($form.find("input[name='standard_code']").length === 0) {
            $form.append("<input type='hidden' name='standard_code' value='" + rowData.standard_code + "'>");
        } else {
            $form.find("input[name='standard_code']").val(rowData.standard_code);
        }
        ...
        const standardCode = selectedOption.data("standard_code");

        if ($form.find("input[name='standard_code']").length === 0) {
            $form.append("<input type='hidden' name='standard_code' value='" + standardCode + "'>");
        } else {
            $form.find("input[name='standard_code']").val(standardCode);
        }
...
function getDataList() {
    dataListTable = new Tabulator("#dataList", {
        ...
        ajaxConfig: "POST",
        ajaxLoader: false,
        ajaxURL: "/chunil/standard/nonTime/list",
        ...
        columns: [
            ...
            {title: "standard_code", field: "standard_code", sorter: "string", width: 250, hozAlign: "center", headerSort: false, visible: false},
            ],
...
function getDataList2() {
    dataViewTable = new Tabulator("#dataView", {
        ...
        ajaxConfig: "POST",
        ajaxLoader: false,
        ajaxURL: "/chunil/standard/nonTime/view",
        ajaxProgressiveLoad: "scroll",
...
function updateCorrStatus() {
    ...
    $.ajax({
        url: "/chunil/standard/nonTime/update",
        type: "POST",
        data: corrForm,
        dataType: "json",
        processData: false,
...
function deleteCorrStatus() {
    ...
    $.ajax({
        url: "/chunil/standard/nonTime/delete",
        type: "POST",
